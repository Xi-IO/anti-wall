...
from wall.render.round_render import RoundData, get_round_data, load_round_data


@dataclass(frozen=True)
class LoadedViewerData:
    ticks: pd.DataFrame
    deaths: pd.DataFrame
    fires: pd.DataFrame
    hurts: pd.DataFrame
    ...
    @classmethod
    def from_data_dir(cls, data_dir: Path) -> "LoadedViewerData":
    ...
    # IMPORTANT: keep dataset-to-round assembly here so app.py stays focused on
    # viewer orchestration instead of carrying raw table plumbing.
    def build_round_data(self, round_id: int, *, tickrate: float) -> RoundData:
